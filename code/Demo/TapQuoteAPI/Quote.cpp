...
void Quote::run()//自定义函数抛弃原先demo的runtest
{
	if (NULL == m_pAPI)
	{
	...
	OnRspSubscribeQuote(m_uiSessionID, iErr, 0, 0);
	while (true) 
{
		m_Event.WaitEvent();
	}
	//const TapAPIQuoteWhole *retmes1;
	//OnRspUnSubscribeQuote(m_uiSessionID, iErr,APIYNFLAG_NO,retmes1 );
